...
### 乐观锁
乐观锁是一种乐观的思想，认为读多写少，遇到并发的可能性低，每次拿数据时都认为别人不会修改，所以不会上锁，但是在更新的时候会判断一下在此期间别人有没有去更新这个数据，可以使用版本号机制和 CAS 算法实现。

cas是乐观锁的实现方式，可以参考资料:[乐观锁的实现机制--CAS（Compare And Swap）](https://blog.csdn.net/qq_35571554/article/details/82892806).  

cas的关键在于不需要阻塞线程，可以持有cpu一直执行程序等待资源释放，不进行内核态和用户态的切换。这种方式也叫做自旋。

### 悲观锁
java中 synchronized 就是一种悲观锁。

ReentrantLock（可重入锁）也是一种悲观锁，底层使用了CAS+AQS队列实现，AQS队列使用了一个先入先出的队列作为基础。
...
### volatile关键字
[资料](https://blog.csdn.net/suifeng3051/article/details/52611233)
...
## Java内存模型（JMM）
[资料](https://blog.csdn.net/suifeng3051/article/details/52611310)
