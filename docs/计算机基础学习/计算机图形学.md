...
用拍照来比喻的话，model变换是选择场景和安排场景和人物的位置；view变换是调整相机的角度和位置；projection变换是将场景中的物体投影至相片上。


### 1.1 点积 dot product
$\vec{a}\cdot \vec{b} = ||\vec{a}|| \cdot ||\vec{b}|| cos\theta$  
代表$\vec{a}$在$\vec{b}$上的投影，或$\vec{b}$在$\vec{a}$上的投影。  \
如果用三维中的具体坐标来表示，点积公式如下：  
$\vec{a}\cdot \vec{b} =  \vec{a}^{T}\cdot \vec{b} = 
\left( \begin{matrix} {x_a} \ {y_a} \ {z_a} \end{matrix} \right) \left( \begin{matrix} {x_b} \\ {y_b} \\ {z_b} \end{matrix} \right) =\left( \begin{matrix} {x_a x_b} + {y_a y_b} + {z_a z_b} \end{matrix} \right)$ 
### 1.2 叉积 cross product
定义：$\vec{a}\times \vec{b}$  
方向:右手法则，四指从$\vec{a}$握向$\vec{b}$，大拇指的方向即为叉积方向  
大小：$||\vec{a}\times \vec{b}|| = ||\vec{a}|| \cdot ||\vec{b}|| sin\theta$
如果用三维中的具体坐标来表示，叉积公式如下：  
$\vec{a}\times \vec{b} =  A\cdot \vec{b} = 
\left( \begin{matrix} 0 \ {-z_a} \ {y_a}\\  {z_a} \ 0 \ {-x_a}\\ {-y_a} \ {x_a} \ 0\end{matrix} \right) \left( \begin{matrix} {x_b} \\ {y_b} \\ {z_b} \end{matrix} \right)$ 
### 1.3 2D变换 (模型变换 Model Transformations)
平移 shift  
旋转 rotation  
缩放 scale   
错位切换 shear   
